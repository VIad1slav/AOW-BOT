...
async def main():
    ...
    print('— предупреждения едут вместе с документами, а не прячутся в логе —')
    warn = b._warnings_block([
        '  📄 фактура собрана',
        '  ⚠ CMR: нет названия груза для кода 34039900 — строка ушла без названия',
        '  ⚠ CMR: нет названия груза для кода 34039900 — строка ушла без названия',
    ])
    check('предупреждение попало в сообщение', 'нет названия груза' in warn, True)
    check('  повтор не задвоился', warn.count('34039900'), 1)
    check('  обычные строки не попали', 'фактура собрана' in warn, False)
    check('без предупреждений блок пустой', b._warnings_block(['  ✅ всё хорошо']), '')
    many = b._warnings_block(['  ⚠ проблема %d' % i for i in range(10)])
    check('длинный список обрезан', many.count('•'), b.WARN_LIMIT + 1)
    check('  и сказано, сколько осталось', 'и ещё 4' in many, True)
